...
module mips_rtype_tb_sm();
  ...
  initial begin
    ...
    // Mostrar encabezado
    $display("\n==== MIPS Pipeline R-Type Instructions Testbench con Máquina de Estados ====\n");
    ...
    case(state)
      ...
      CHECK_END_STATE: begin
        if (halt) begin
          ...
          // Verificación final de resultados para instrucciones R-Type
          $display("\n==== VERIFICACIÓN FINAL AL DETECTAR HALT (Ciclo %0d) ====", cycle_count);
          $display("Registros finales:");
          $display("$1=%0d (Esperado: 10)", register_values[1]);
          $display("$2=%0d (Esperado: 20)", register_values[2]);
          $display("$3=%0d (Esperado: 10 & 20 = 0)", register_values[3]);
          $display("$4=%0d (Esperado: 10 | 20 = 30)", register_values[4]);
          $display("$5=%0d (Esperado: 10 ^ 20 = 30)", register_values[5]);
          $display("$6=%0d (Esperado: ~(10 | 20) = -31)", register_values[6]);
          $display("$7=%0d (Esperado: (10 < 20) ? 1 : 0 = 1)", register_values[7]);
          $display("$8=%0d (Esperado: 20 << 2 = 80)", register_values[8]);
          $display("$9=%0d (Esperado: 20 >> 2 = 5)", register_values[9]);
          $display("$10=%0d (Esperado: 10 + 20 = 30)", register_values[10]);
          $display("$11=%0d (Esperado: 20 - 10 = 10)", register_values[11]);
          $display("$12=%0d (Esperado: (10u < 20u) ? 1 : 0 = 1)", register_values[12]);
          $display("$14=%0d (Sin valor esperado específico)", register_values[14]);
          $display("$15=%0d (Esperado: 2)", register_values[15]);
          $display("$16=%0d (Esperado: 100 - dirección base para memoria)", register_values[16]);
          $display("$17=%0d (Esperado: 20 << 3 = 160 - SLLV)", register_values[17]);
          $display("$18=%0d (Esperado: 20 >> 2 = 5 - SRLV)", register_values[18]);
          $display("$19=%0d (Esperado: -3 >>> 2 = -1 - SRAV)", register_values[19]);
          $display("$20=%0d (Esperado: 2 - cantidad desplazamiento)", register_values[20]);
                 
          // Verificar memoria usando los valores leídos por debug
          $display("\nMemoria final:");
          $display("Mem[100]=%0d", mem_values[0]);
          $display("Mem[104]=%0d", mem_values[1]);
          $display("Mem[35]=%0d", mem_values[2]);
          $display("Mem[39]=%0d", mem_values[3]);
                 
          // Verificar resultado para instrucciones R-Type
          if (register_values[3] == 0 &&
              register_values[4] == 30 &&
              register_values[5] == 30 &&
              register_values[6] == -31 &&
              register_values[7] == 1 &&
              register_values[8] == 80 &&
              register_values[9] == 5 &&
              register_values[10] == 30 &&
              register_values[11] == 10 &&
              register_values[12] == 1 &&
              register_values[15] == 2 &&
              register_values[17] == 160 &&
              register_values[18] == 5 &&
              register_values[19] == -1) begin
  ...
  initial begin
    $dumpfile("mips_rtype_sm.vcd");
    $dumpvars(0, mips_rtype_tb_sm);
  end
